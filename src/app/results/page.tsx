...
function ResultsContent() {
  ...
  function exportCSV() {
    const header = ["sellerId","itemId","title","priceValue","priceCurrency","watchCount","url","listedAt"];
    ...
    const csv = [header, ...rows]
      .map((r) => r.map((v) => (typeof v === "string" && v.includes(",")) ? `"${v.replaceAll('"', '""')}"` : String(v)).join(","))
      .join("\n");
    ...
    <div className="min-h-screen px-6 py-10 max-w-7xl mx-auto">
      <header className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={exportCSV} disabled={items.length === 0} className="rounded-lg border py-2 px-3 input-glass hover:bg-white/40 transition disabled:opacity-50">CSV</button>
          <button onClick={exportXLSX} disabled={items.length === 0} className="rounded-lg border py-2 px-3 input-glass hover:bg-white/40 transition disabled:opacity-50">Excel</button>
        </div>
      </header>
              ...
              {items.map((it, idx) => (
                <tr key={`${it.itemId}-${idx}`} className="odd:bg-white/40 even:bg-white/20">
                  ...
                  <td className="px-2 py-2 text-right whitespace-nowrap">
                    {it.priceValue != null && it.priceCurrency ? `${it.priceValue} ${it.priceCurrency}` : "-"}
                  </td>
                  <td className="px-2 py-2 text-center">{typeof it.watchCount === "number" ? it.watchCount : "—"}</td>
                  <td className="px-2 py-2 max-w-[280px] truncate">
                    {it.url ? (
                      <a className="text-blue-700 underline" href={it.url} target="_blank" rel="noreferrer">Link</a>
                    ) : (
                      "-"
                    )}
                  </td>
